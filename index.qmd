...
Quarto allows the user an easy way to insert citations, especially for R packages. With the cursor in the place you wish to insert a citation, choose from the `Insert` menu on the menu at the top of the tab, and select `Citation...`. Also, in the citation window at the bottom you may change the name of the bibliography file (`.bib`) in the `Create` field. The citation feature of Quarto is not only useful for citing R packages but may be used to cite any published material from the scientific literature. For other sorts of citations references may be found by selecting `From DOI` or `PubMed` in the citation window. Adding citations will create a bibliography of all the references that will appear at the end of the rendered document.
...
Before starting your analyses, you will need to load the package libraries (make sure those packages are installed first). Remember to add a unique label (`#| label:`) at the top of each block of code (see **Create an R environment**).
...
Often it is good to start a particular analysis with a summary of the methodology. If you do this as you go along then once you are ready to write up your results in a manuscript the methods section will write itself. Remember to be detailed. You have a little more freedom in supplemental material. You can add more detailed descriptions compared to the methods section of a published paper. Here is an example outlining the methodology behind the Asian White-eye morphometrics example we will perform next.
